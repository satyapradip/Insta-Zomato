    ...
    <div className="max-w-4xl w-full mx-auto p-4 md:p-8 space-y-6">
      ...
      <div className="space-y-4">
        {PAST_ORDERS.map((order) => (
          <div
            key={order.id}
            className={`bg-card border rounded-3xl p-5 md:p-6 space-y-4 shadow-xs transition-all ${
              order.isLive
            ...
            <div className="flex items-center justify-between pt-3 border-t border-border">
              ...
              {order.isLive ? (
                <Link
                  ...
                  className="bg-primary hover:bg-primary-hover text-white text-xs font-bold py-2 px-4 rounded-xl flex items-center gap-1.5 shadow-xs transition-all cursor-pointer"
                >
                  <Bike className="w-3.5 h-3.5" />
                  <span>Track Live GPS Route ➔</span>
                </Link>
              ) : (
                <button className="text-xs font-semibold text-muted hover:text-foreground bg-card-elevated hover:bg-card-hover border border-border px-3 py-1.5 rounded-xl transition-colors shadow-xs cursor-pointer">
                  Re-Order Dishes
                </button>
              )}
            </div>
